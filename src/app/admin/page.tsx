...
// Schemas for the Settings tab, broken down for individual forms
...
function AddCommitteeForm({ onAdd }: { onAdd: (data: any) => Promise<void> }) {
    ...
    return <Form {...form}><form onSubmit={form.handleSubmit(async (d) => { await onAdd(d); form.reset(); })} className="space-y-4">
        <div className="grid md:grid-cols-2 gap-4">
         ...
         <FormField control={form.control} name="chairImageUrl" render={({ field }) => ( <FormItem><FormLabel>Chair Image URL</FormLabel><FormControl><Input {...field} /></FormControl><FormDescription>Use a standard image URL or a Google Drive "share" link.</FormDescription></FormItem> )} />
...
export default function AdminPage() {
  ...
  const generalSettingsForm = useForm<z.infer<typeof generalSettingsSchema>>({ resolver: zodResolver(generalSettingsSchema) });
  const socialLinksForm = useForm<z.infer<typeof socialLinksSchema>>({ resolver: zodResolver(socialLinksSchema) });
