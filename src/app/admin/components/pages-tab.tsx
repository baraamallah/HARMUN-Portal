...
function HighlightItemForm({ item, onSave, onDelete }: { item: T.ConferenceHighlight; onSave: (id: string, data: z.infer<typeof highlightItemSchema>) => void; onDelete: (id: string) => void }) {
  ...
  const handleSave = () => {
    form.handleSubmit((data) => onSave(item.id, data))();
  };

  return (
    <Form {...form}>
      <div className="flex flex-wrap gap-2 items-start p-2 border rounded-md mb-2">
        ...
        <div className="flex gap-1 pt-6"><Button type="button" size="sm" onClick={handleSave}>Save</Button><Button size="sm" variant="destructive" type="button" onClick={() => onDelete(item.id)}><Trash2 className="h-4 w-4"/></Button></div>
      </div>
    </Form>
  );
}
...
function AddHighlightForm({ onAdd }: { onAdd: (data: any, form: any) => Promise<void> }) {
    ...
    return <Form {...form}><div onSubmit={form.handleSubmit((d) => onAdd(d, form))} className="space-y-4 p-4 border-t">
    ...
    return (
        <div className="space-y-6">
            <SectionCard title="Home Page" description="Manage the content for your site's landing page." icon={Home}>
                <Form {...homeForm}>
                    <div onSubmit={homeForm.handleSubmit((d) => handleAction(firebaseService.updateHomePageContent(d), "Home page content updated."))} className="space-y-6">
             ...
             <SectionCard title="About Page" description="Edit the content for the about page." icon={FileBadge}>
                <Form {...aboutForm}><div onSubmit={aboutForm.handleSubmit((d) => handleAction(firebaseService.updateAboutPageContent(d), "About page content updated."))} className="space-y-6">
             ...
             <div className="grid md:grid-cols-2 gap-6">
                 <SectionCard title="Registration Page" description="Manage page titles and subtitles." icon={UserSquare}>
                    <Form {...registrationForm}><div onSubmit={registrationForm.handleSubmit((d) => handleAction(firebaseService.updateRegistrationPageContent(d), "Registration page updated."))} className="space-y-4">
                ...
                <SectionCard title="Documents Page" description="Manage downloadable files." icon={Book}>
                    <Form {...documentsForm}>
                        <div onSubmit={documentsForm.handleSubmit((d) => handleAction(firebaseService.updateDocumentsPageContent(d), "Documents page updated."))} className="space-y-4">
