...
function AddHighlightForm({ onAdd }: { onAdd: (data: any, form: any) => Promise<void> }) {
    ...
    return <Form {...form}>
      <div className="space-y-4 p-4 border-t">
        ...
        <Button type="button" onClick={form.handleSubmit((d) => onAdd(d, form))} className="w-full"><PlusCircle className="mr-2"/>Add Highlight</Button>
      </div>
    </Form>;
}
...
function AddDownloadableDocumentForm({ onAdd }: { onAdd: (data: any, form: any) => Promise<void> }) {
    ...
    return <Form {...form}>
      <div className="space-y-4 p-4 border-t">
        ...
        <Button type="button" onClick={form.handleSubmit((d) => onAdd(d, form))} className="w-full"><PlusCircle className="mr-2"/>Add Document</Button>
    </div>
    </Form>;
}
    ...
    return (
        <div className="space-y-6">
            <SectionCard title="Home Page" description="Manage the content for your site's landing page." icon={Home}>
                <Form {...homeForm}>
                    <form onSubmit={homeForm.handleSubmit((d) => handleAction(firebaseService.updateHomePageContent(d), "Home page content updated."))} className="space-y-6">
